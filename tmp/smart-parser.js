...
exports.parseAutor = parseAutor;
exports.parseExtraInfo = parseExtraInfo;
exports.extractTipo = extractTipo;
...
function parseAutor(autor) {
    ...
    try {
        ...
        if (proto)
            info.protocolo = proto[1].trim();
        // Funcionário (when present)
        const func = RE_FUNC.exec(autor);
        if (func)
            info.funcionario = func[1].trim();
        // Contrato Filho (SUB XX)
        const cSub = RE_CONTRATO_SUB.exec(autor);
        if (cSub) {
            const sub = (_a = cSub[1]) === null || _a === void 0 ? void 0 : _a.trim();
            if (sub)
                info.contratoFilho = sub.replace(/\s*-\s*$/, '');
        }
        else {
        ...
        const escopo = (escopoPipe === null || escopoPipe === void 0 ? void 0 : escopoPipe[1]) || (escopoRegex === null || escopoRegex === void 0 ? void 0 : escopoRegex[1]);
        if (escopo)
            info.escopo = escopo.trim();
    }
    catch {
        // ignore
    }
...
function tryParseBrDate(s) {
    ...
    for (const p of patterns) {
        try {
            const d = (0, date_fns_1.parse)(s, p, new Date());
...
function tryParseEnShort(s) {
    const withYear = `${s}, ${new Date().getFullYear()}`;
    const patterns = ['LLL dd, HH:mm, yyyy'];
    for (const p of patterns) {
        try {
            const d = (0, date_fns_1.parse)(withYear, p, new Date());
            if ((0, date_fns_1.isValid)(d))
                return d;
        }
        catch { }
...
function extractPrazoDate(line, nextLine) {
    const trimmed = line.trim();
    // Prefer BR absolute date
    const br = tryParseBrDate(trimmed);
    if (br)
        return { prazoText: trimmed, prazoDate: br };
...
function parseExtraInfo(extraInfo) {
    ...
    while (i < lines.length) {
        ...
        items.push({ acao, responsavel, prazoText, prazoDate: prazoDate !== null && prazoDate !== void 0 ? prazoDate : null, deltaDias });
...
function extractTipo(dataField) {
